...
Page({
    ...
    updateBBS() {
        ...
        buckyhelper.getRoomInfo(that.data.id, function(roominfo) {
            if (roominfo) {
                BX_INFO("get roominfo ", roominfo, getCurrentTraceInfo());
                if (roominfo.bbs) {
                ...
                BX_ERROR('could not get room info, by id', that.data.id, getCurrentTraceInfo());
            }
        }, true);
    },
    onLoad: function(query) {
        ...
        BX_INFO('attach event', eventName, getCurrentTraceInfo());
        em.attach(eventName, function(msg) {
            BX_INFO('on room_event', msg, getCurrentTraceInfo());
            ...
            BX_INFO('attach event callback ', result, unknown, getCurrentTraceInfo());
        })
        //主动更新一次公告
        that.updateBBS();
    ...
    OnHistoryScroll: function(event) {
        BX_INFO(event.detail, getCurrentTraceInfo());
    },
    ...
    FetchMessage: function(begin, num) {
        ...
        buckyhelper.getChatRoomModule(function(chatroom) {
            chatroom.getHistoryCount(buckyhelper.getSessionID(), that.data.id, function(count) {
                ...
                BX_INFO('get history list, start', start, 'end', end, getCurrentTraceInfo());
                chatroom.getHistoryList(buckyhelper.getSessionID(), that.data.id, start, end, function(historyList) {
                    BX_INFO('get history list callback', historyList, getCurrentTraceInfo());
                        ...
                        that.getHistoryInfoByHistoryList(history, historyList.ret, function(infos) {
                            ...
                            function getUserInfoCallback(userinfo) {
                                ...
                                if (count >= infos.length) {
                                    ...
                                    BX_INFO("history infos ", infos, getCurrentTraceInfo());
                                    ...
                                    if (infos.length > 0) {
                                        that.setData({
                                            lastviewid: "id_" + infos[infos.length - 1].id
                                        });
                                    }
    ...
    onSubmitForm: function(e) {
        BX_INFO(e.detail.value.message, getCurrentTraceInfo());
        let content = e.detail.value.message;
        let that = this;
        let openid = buckyhelper.getSessionID();
        buckyhelper.getHistoryModule(function(history) {
            console.assert(history != null)
            BX_INFO('add history', that.data.roomInfo.id, openid, content, getCurrentTraceInfo());
            history.addHistory(buckyhelper.getSessionID(), that.data.roomInfo.id, openid, '', content, 1, function(result) {
                ...
                BX_INFO('add history result', result, getCurrentTraceInfo());
                if (result.ret) {
                    that.setData({
                        inputValue: ""
                    });
    ...
    getHistoryInfoByHistoryList: function(history, historyList, cb) {
        BX_INFO('getHistoryInfoByHistoryList', historyList, getCurrentTraceInfo());
    ...
    OnEnterAdminRoom: function(event) {
        wx.navigateTo({
            url: `../chatroom/adminroom?id=${this.data.id}&name=${this.data.roomInfo.name}&admin=${this.data.isAdmin}`
        });
    },
